"""OCRmyPDF pluggy hooks and OcrEngine implementation for ocr-demo."""
...
from ocrmypdf_ocrdemo.client import post_ocr_sync
from ocrmypdf_ocrdemo.config import build_ocrdemo_options_from_extra, get_ocrdemo_options
...
def _ocr_tree_to_hocr(page: OcrElement) -> str:
    ...
    chunks: list[str] = [
        ...
        "<meta name='ocr-system' content='ocrmypdf-ocrdemo'/>",
        "</head>",
        "<body>",
...
class OcrdemoOcrEngine(OcrEngine):
    """OCR engine that delegates recognition to an ocr-demo HTTP service."""

    @staticmethod
    def version() -> str:
        return "ocr-demo-remote"

    @staticmethod
    def creator_tag(options: OcrOptions) -> str:
        return "OCRmyPDF fpdf2 + ocr-demo HTTP API"

    def __str__(self) -> str:
        return "ocr-demo (HTTP)"
    ...
    def get_orientation(input_file: Path, options: OcrOptions) -> OrientationConfidence:
        ...
        try:
            ...
            log.warning("ocrdemo get_orientation failed: %s", exc)
            return OrientationConfidence(angle=0, confidence=0.0)
    ...
    def get_deskew(input_file: Path, options: OcrOptions) -> float:
        ...
        try:
            ...
            log.warning("ocrdemo get_deskew failed: %s", exc)
            return 0.0
    ...
    def generate_hocr(
        ...
        tree, text = OcrdemoOcrEngine.generate_ocr(input_file, options, page_number=0)
...
def check_options(options: OcrOptions) -> None:
    if getattr(options, "ocr_engine", None) != "ocrdemo":
        return
    try:
        oc = build_ocrdemo_options_from_extra(options)
    except ValidationError as exc:
        raise BadArgsError(f"Invalid ocr-demo options: {exc}") from exc
    if oc.base_url is None:
        raise BadArgsError(
            "When using --ocr-engine ocrdemo, you must set --ocrdemo-base-url "
            "to the root URL of a running ocr-demo instance (e.g. http://127.0.0.1:8000)."
        )
    options.extra_attrs["_ocrdemo"] = oc


@hookimpl
def get_ocr_engine(options: OcrOptions | None) -> OcrdemoOcrEngine | None:
    if options is None:
        return None
    if getattr(options, "ocr_engine", None) != "ocrdemo":
        return None
    return OcrdemoOcrEngine()
